...
% Real author list pending team sign-off. Do not submit with these
% placeholders in place. CIDR is SINGLE-BLIND, so real names go here.
% =========================================================================
\author{Anonymous Placeholder}
\authornote{Real author list pending team sign-off.
  Do not submit with placeholder values in place.}
\affiliation{%
...
\texttt{as\_of\_valid(entity, attr, t)} answers ``what was true at $t$''; \texttt{as\_of\_believed(entity, attr, t, s)} answers ``what did we believe at $s$ about time $t$'' --- the shape of question point-in-time regulatory reporting requires~\cite{bcbs} and, combined with the audit relation, the shape a lender needs to defend a past decision.
...
\noindent\textbf{Enforcement.} Table~\ref{tab:adv} shows the headline: the plain schema accepts every violation; carefully written application guards catch fewer than half; both engine-resident approaches catch all of them. We emphasize the steelman's 70/70 rather than hiding it: the guarantee is achievable by hand, provided every application, on every table, re-implements it correctly, forever. The 30/70 row is what that assumption looks like in practice.
...
\noindent\textbf{Positioning.} Every mechanism in KNDB has ancestry. Our contribution is the position that origin should be a first-class, engine-enforced property governing writes, conflicts, and use in a general-purpose store, together with evidence that the composition is buildable in 508 lines on stock infrastructure, with measurable enforcement value and measured, structured cost.
...
\begin{acks}
Portions of this manuscript were prepared with the assistance of AI tools for language editing and formatting. All research content, ideas, experimental design, code, and citations are the authors' work, and the authors take full responsibility for the final content in accordance with the ACM Policy on Authorship for Generative AI Tools.
\end{acks}
...
\bibitem{memir}
Authors. 2026. MemIR: typed memory representations for agent retrieval. \emph{arXiv preprint arXiv:2605.25869}.

\bibitem{gsm}
Authors. 2026. Governed shared memory for multi-agent systems. \emph{arXiv preprint arXiv:2606.24535}.
...
\bibitem{codec}
Authors. 2025. CoDeC: detecting data contamination in LLMs via in-context learning. \emph{arXiv preprint arXiv:2510.27055}.
