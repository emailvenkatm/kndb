% KNDB -- arXiv preprint version
% Class: plain acmart sigconf with nonacm to suppress conference
% headers/footers and copyright block. acmart ships with TeX Live and
% is available on arXiv without bundling.
...
% ---- Rights / conference: suppressed for arXiv preprint via `nonacm`. ---
...
\renewcommand{\shortauthors}{Maguluri et al.}
...
\section{Evaluation}
\noindent\textbf{Setup.} PostgreSQL 17.10 and ProvSQL 1.10.0, native ARM64 (Apple silicon), no emulation; seed 42; adversarial suite of 100 write payloads (70 trust-violating, 30 legal) executed 10 times; dataset for loaded runs is 11,637 Synthea~\cite{synthea} synthetic patients yielding 565,587 typed facts. Four systems: \texttt{kndb}; \texttt{pg\_naive} (plain schema); \texttt{py\_guards} (application-layer checks in Python, 41 LOC); \texttt{pg\_handrolled} (a steelman: hand-written triggers reproducing the typing and lattice rules, 124 LOC, no confidence propagation). KNDB's engine totals 508 LOC, of which 214 are guard logic.
...
\bottomrule
\end{tabular}
\caption{Adversarial correctness (caught/70) and throughput/latency, both on an empty database, native hardware. Latency and rps are from a uniform-write benchmark, not the adversarial mix.}
\label{tab:adv}
\end{table}
...
\section{Conclusion}
Databases learned long ago that guarantees which depend on every application behaving are not guarantees; that is why types, constraints, and referential integrity live in the engine. We have argued that a fact's epistemic kind now belongs on that list. Model-generated writes now reach shared stores at scale through enrichment pipelines and agent workflows. The costs of conflating them with verified measurements are documented: master-data failures where CRM updates displace postal-verified addresses, and adverse-action decisions creditors cannot justify because the store never recorded which input was measured. Training corpora face a related problem, now handled by external decontamination pipelines because the store itself has no origin concept. KNDB shows the primitive set is small, buildable on stock PostgreSQL in 500 lines, exact where it claims exactness, and honest about what it costs. The mechanisms are ordinary. The position we ask the community to debate is that the engine should know how a value came to exist.
...
\bibitem{cfpb2025}
Consumer Financial Protection Bureau. 2026. Consumer Response Annual Report 2025. March 2026. \url{https://files.consumerfinance.gov/f/documents/cfpb_2025-cr-annual-report_2026-03.pdf}.
