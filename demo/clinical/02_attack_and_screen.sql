...
SELECT
  0 AS depth,
  count(*) FILTER (WHERE epistemic_kind = 'observation') AS n
FROM kndb.fact WHERE upper(sys_time) = 'infinity'
UNION ALL
SELECT 1, count(*) FROM kndb.fact
  WHERE upper(sys_time) = 'infinity' AND epistemic_kind IN ('observation','inference')
UNION ALL
SELECT 2, count(*) FROM kndb.fact
  WHERE upper(sys_time) = 'infinity';
