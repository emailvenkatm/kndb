...
\begin{itemize}
\item \texttt{ALTER TABLE \dots DISABLE TRIGGER ALL} sets \texttt{pg\_trigger.tgenabled = 'D'} (\texttt{tablecmds.c:5588-5592}); \texttt{TriggerEnabled} at \texttt{trigger.c:3491-3499} returns false. A trigger-based enforcer is bypassable here; a TAM callback is not.
\item \texttt{SET session\_replication\_role = 'replica'}. \texttt{TriggerEnabled} at \texttt{trigger.c:3489-3499} skips \texttt{TRIGGER\_FIRES\_ON\_ORIGIN} under \texttt{SESSION\_REPLICATION\_ROLE\_REPLICA}. The GUC is \texttt{PGC\_SUSET}. Trigger bypassable; TAM callback not.
...
The engine-in-storage differentiator holds against a writer with \texttt{INSERT} plus \texttt{ALTER TABLE} on the target relation, or a role that can toggle \texttt{session\_replication\_role} (a replication or CDC operator, a migration tool, a connection pool). It does not hold against the table owner.
...
KNDB is a \texttt{shared\_preload\_libraries} extension of 1479 lines of C in \texttt{src/} plus 310 lines of headers in \texttt{include/} (1789 total per \texttt{wc -l}). It targets PostgreSQL 18 REL\_18\_STABLE via PGXS. Building the extension against a stock 18.4 install requires no PostgreSQL patch.

\subsection{Handler and delegation}
The AM handler copies the heap AM's \texttt{TableAmRoutine} at first call and overrides three entries:
\begin{itemize}
...
\end{itemize}
The remaining $\sim$37 callbacks (\texttt{scan\_begin}, \texttt{scan\_getnextslot}, \texttt{tuple\_update}, \texttt{tuple\_delete}, \texttt{tuple\_lock}, \texttt{index\_fetch\_*}, \texttt{relation\_set\_new\_filelocator}, \texttt{relation\_copy\_data}, \texttt{relation\_vacuum}, and so on) are heap's, unmodified. Reads, updates, deletes, index builds, VACUUM, and analyse all inherit heap's behaviour byte-for-byte.
...
Table~\ref{tab:bypass} summarises the F2 and F18 bypass-survival results. Each cell corresponds to a scenario in \texttt{scripts/bypass.sh}, which asserts against both an epistemic table (\texttt{fact\_native}) and a plain-heap table with a BEFORE INSERT trigger (\texttt{fact\_trigger}). The adversarial disable-and-test rebuilds the extension with the \texttt{multi\_insert} wiring line commented out and confirms the COPY-FROM bypass returns; dylib SHA-256 flips \texttt{eb15d442\ldots} $\to$ \texttt{302bb93b\ldots} $\to$ \texttt{eb15d442\ldots}.

\begin{table}[t]
\centering
\caption{Bypass scenarios and outcomes. From \texttt{scripts/bypass.sh} transcripts and README threat-model enumeration.}
\label{tab:bypass}
\small
...
\textbf{Three of $\sim$40 TAM callbacks overridden.} We override \texttt{tuple\_insert}, \texttt{multi\_insert}, and \texttt{relation\_toast\_am}. Every other callback (scan, update, delete, index build, VACUUM, analyse, replication copy, cluster, tuple lock, ttids, and so on) is heap's, unmodified. We inherit heap's behaviour on all of them, including its bugs. This is deliberate; the AM is a write-time enforcement point, not a storage-format replacement.
