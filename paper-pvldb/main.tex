...
\node[cb, right=0.15cm of tcs]   (rta) {relation\_\\toast\_am};
...
% Bypass arrows: skip the envelope, go straight to heap
\draw[arb] (setam.west)   to[out=180, in=0, looseness=0.4]   (heap.east);
\draw[arb] (trunc.west)   to[out=180, in=15, looseness=0.5]  (heap.east);
\draw[arb] (cluster.west) to[out=180, in=-15, looseness=0.5] (heap.east);

\end{tikzpicture}
\caption{Write-path routing to the epistemic TAM. Six user-space SQL paths (top) reach one of the seven overridden callbacks (middle envelope); the callback runs R1--R5, the F6 advisory xact lock, precedence with the F8 xmin tiebreak, and the eviction audit before delegating storage to \texttt{heapam}. Three DDL-privileged paths (right, dashed) bypass the envelope entirely; they are out of scope (Section~\ref{sec:limitations}).}
\label{fig:writepath}
\end{figure*}
